...
`sourcePath` 是解析入口路径。`mode` 标记输入构造身份。`parser` 标记 official parser 名称和版本。`children` 保存文档结构树。`targets` 保存本地 target catalog。`xrefOccurrences` 和 `anchorOccurrences` 保存源文档中的 inline occurrence。`toolDiagnostics` 保存解析器诊断。

本书定义的解析器输出使用 `mode: "single-file"`。xref:../10-book-entry-source-mapped-logical-document/book.adoc[书稿入口回源逻辑文档规约] 定义的 book-entry 输入构造要求 `AbundantDocument` 接受 `book-entry` mode，并保持节点、target、occurrence 和 diagnostic 语义不变。
