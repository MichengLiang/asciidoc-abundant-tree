...
节点模型描述 AsciiDoc source document 中可恢复的结构事实。每个节点可以携带 source layer、official layer 和子节点。
...
Title 保存标题文本和可恢复的 source location。没有源标题行时，解析器可以保留 official title text，但不得伪造 raw source。
...
`ParagraphNode` 表示 official paragraph block。Paragraph 可以保存 text，并可以包含 xref 或 anchor occurrence 子节点。

Paragraph 不恢复完整 inline formatting CST。strong、emphasis、literal inline formatting 等表面在当前对象中作为文本保留。
...
Table 保存 explicit ID、block title、metadata、rows、cells 和 source span。Table 内容结构来自 Asciidoctor official table model；解析器不手写 table grammar。

=== Metadata

`MetadataNode` 表示附着到 block 或 section 的源文本 metadata surface。

当前 metadata kind 包括 `id`、`title`、`attrlist`、`anchor` 和 `unknown`。
...
Anchor occurrence 与 target catalog entry 是不同投影。一个 anchor 可以产生 target fact，但 occurrence 仍然保留其源文本位置。

=== Target

`TargetNode` 表示当前文件内可被引用的本地 target。
