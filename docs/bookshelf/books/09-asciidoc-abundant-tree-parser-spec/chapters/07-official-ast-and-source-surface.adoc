...
源码扫描可以补充 metadata、raw、line span、column span、局部 delimiter confirmation 和 fallback diagnostic。
...
Paragraph 和可扫描 table prose 可以产生 xref 或 anchor occurrence。Listing、literal、pass-through 和 fenced code content 不产生 xref 或 anchor occurrence。

Open block 是容器。它自身不产生 prose occurrence；其 paragraph descendants 可以产生 occurrence。
