...
`aat:addressLabel` 来自标题自身手动 ID，或来自归属到标题的本地非标题 target ID。selector 指向 listing、table、block、inline anchor 或 payload block 的本地 target ID 时，target heading 是该 target ID 所属的标题节点。
...
`payload` 是控制字段，用于绑定边 payload。它不重复作为普通属性。边证据使用 `aat:payloadSelector` 保存该 selector 字符串，并在唯一匹配 source payload id 时使用 `aat:payload` 连接边 payload object。

xref target selector 和 xref payload selector 使用不同绑定空间。xref target selector 查询 heading projection label space；xref payload selector 查询 source payload id space。相同字符串在两个字段中出现时，由字段语境决定解释。
