...
样例中的 `delivery` 和 `capacity` 是标题 address label。`.policy` 和 `.rule` 是标题 role。`.banana` 和 `.pear` 是 payload role，表示作者与下游消费者约定的 schema 或 model label。节点 payload 绑定来自 `for=delivery`。边 payload 绑定来自 xref 的 `payload=rel-delivery` 和 payload block 的 source payload id `rel-delivery`。
...
`[.banana, for=delivery]` 表达节点 payload。`.banana` 是 payload role。`for=delivery` 是节点 payload binding marker 和 heading selector。该 payload block 没有 source payload id；节点 payload 不要求 source payload id。
...
节点 payload 的公共读取路径是 heading 的 `aat:payload`。消费者不需要 source payload id 来读取该 payload。
...
`payload=rel-delivery` 表达边 payload selector。`[#rel-delivery.pear]` 提供该 selector 的 source payload id 和 payload role。边 payload 归属于 xref edge evidence，不归属于 source heading 或 target heading。
