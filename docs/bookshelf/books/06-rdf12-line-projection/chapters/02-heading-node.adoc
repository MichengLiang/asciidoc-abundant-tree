...
标题节点的 IRI 由投影器生成。手动 ID、generated ID 和 headline 不构成节点 IRI。
...
`delivery-policy` 是手动声明的 address label。`配送策略` 是 headline。二者都不构成 `:heading-l5-o0` 的身份。

=== Label 空间

标题节点 label 空间由 `aat:addressLabel`、`aat:generatedAddressLabel` 和 `aat:headline` 构成。selector 绑定标题节点时只查询这三类 label，不查询标题节点 IRI、payload id、role、普通字段谓词或 raw 文本。
...
named attributes 直接使用 `aat:` 命名空间下同名谓词。字段值作为 literal 保存。投影器不解释 `kind`、`status`、`owner` 的业务语义。
